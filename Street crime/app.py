import os, secrets, time, datetime
...
# SocketIO with Redis message queue for multi-instance scaling
...
class User(db.Model):
    ...
    role = db.Column(db.String(30), default='player')  # player/helpdesk/moderator/admin
...
# Utility: generate short-lived admin token saved in Redis
...
if __name__ == '__main__':
    # create DB + sample data if missing
    db.create_all()
    if not User.query.filter_by(username='admin').first():
        ...
        db.session.commit()
        print('Created sample users: admin/mod/help/player1/player2 (passwords are adminpass/modpass/helppass/player1/player2)')
